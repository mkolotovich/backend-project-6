...
export default (app) => {
  app
    .get('/tasks', { name: 'tasks' }, app.fp.authenticate('page', async (req, reply, err, user) => {
      if (err) {
    ...
    .get('/tasks/new', { name: 'newTask' }, app.fp.authenticate('page', async (req, reply, err, user) => {
      if (err) {
        return app.httpErrors.internalServerError(err);
      }
      if (!user) {
    ...
    .post('/tasks', app.fp.authenticate('page', async (req, reply, err, user) => {
      if (err) {
        return app.httpErrors.internalServerError(err);
      }
      if (!user) {
        ...
        try {
          ...
          const taskCreator = await app.objection.models.user.query().findById(newTask.creatorId);
          await app.objection.models.label.transaction(async (trx) => {
            const taskUser = await taskCreator.$relatedQuery('owner', trx).insert({
              name: newTask.name,
    ...
    .get('/tasks/:id', app.fp.authenticate('page', async (req, reply, err, user) => {
      if (err) {
        return app.httpErrors.internalServerError(err);
      }
      if (!user) {
    ...
    .get('/tasks/:id/edit', app.fp.authenticate('page', async (req, reply, err, user) => {
      if (err) {
        return app.httpErrors.internalServerError(err);
      }
      if (!user) {
    ...
    .patch('/tasks/:id', app.fp.authenticate('page', async (req, reply, err, user) => {
      if (err) {
        return app.httpErrors.internalServerError(err);
      }
      if (!user) {
    ...
    .delete('/tasks/:id', app.fp.authenticate('page', async (req, reply, err, user) => {
      if (err) {
        return app.httpErrors.internalServerError(err);
      }
      if (!user) {
